...
#define PRG_ROM_BLOCK_SIZE 0x4000
#define CHR_ROM_BLOCK_SIZE 0x2000
...
err_t cart_load_rom(const char* rom_filename, cart_t* cart) {
    ...
    file_t* file = open_file(rom_filename, "rb");
    nes_header_t header;
    ...
    cart->pgr_rom = pn_malloc(cart->pgr_size);
    cart->chr_rom = pn_malloc(cart->chr_size);
...
void cart_free(cart_t* cart) {
    if (cart != NULL) {
        pn_free(cart->pgr_rom);
        pn_free(cart->chr_rom);
        pn_free(cart);
    }
}